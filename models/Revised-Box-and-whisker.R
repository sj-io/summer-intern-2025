...
ggplot(plot_data_2.1, aes(x = period, y = di, fill = treated)) +
  geom_boxplot(position = position_dodge(width = 0.75), width = 0.65, outlier.shape = 4, color = "black") +
  scale_fill_brewer("LIHTC in the next decade", palette="Set3") +
  labs(
...
ggsave("output/Q1.1-boxplot.png", width = 9, height = 6, dpi = 300)
...
ggplot(delta_long, aes(x = period, y = delta_di, fill = treated)) +
  geom_boxplot(position = position_dodge(width = 0.7), width = 0.55, outlier.shape = NA) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkred") +
  theme_bw(base_size = 13) +
  scale_fill_manual(
    values = c("No LIHTC" = "#a1d76a", "LIHTC" = "#e9a3c9")) +
  labs(
    title = "Change in Signed DI by Decade and LIHTC Treatment",
    subtitle = "Grouped by whether tract received LIHTC in the given decade",
    x = "Decade",
    y = "Change in DI (ΔDI)",
...
ggsave("output/Q2.1-box.png", width = 9, height = 6, dpi = 300)
