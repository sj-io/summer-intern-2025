...
# This is me defining the different variables 
all_states <- c(state.abb, "DC", "PR")
housing_data <- NULL 
# This gives me the different states 
for(st in all_states) { 
  message(paste("Downloading data for", st, ".."))
  # This lets me know when the data is being downloaded per state 
state_data <-tryCatch ({ 
  get_acs( 
  geography = "tract",
  variables= acs_vars,
  ...
  output= "wide",
  state= st
  # This gives me the acs 5 year survey from 2022 and gives me the data i need
) 
}, error= function(e) { 
...
housing_data <- housing_data %>%
  mutate(
    # I used E to show when there was estimation and i define rental share and rental deserts 
    rental_share = (renter_occupiedE + vacant_for_rentE) / total_unitsE, 
    rental_desert = rental_share < 0.2, # the rental share is less than 20 percent
    extreme_rental_desert = rental_share < 0.10 # the rental share is less than 10 percent
  )
...
summary_replication <- housing_data %>%
  summarise(
    ...
    percentage_rental_deserts = (number_rental_deserts / total_nation_tracts) * 100,
